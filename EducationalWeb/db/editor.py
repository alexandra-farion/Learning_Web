...
from EducationalWeb.models import Peoples, Teachers, Students, Classes, Diary
...
async def add_teachers():
    with open(teachers_csv, encoding='utf-8') as file:
        for teacher_and_classes in csv.reader(file, delimiter=";"):
            ...
            people, created = await Peoples.objects.aget_or_create(nickname=teacher, name=teacher, password="1111",
                                                                   school='МАОУ "Лицей №6"')

            await Teachers.objects.aget_or_create(nickname=people, character="teacher",
...
async def add_students():
    with open(students_csv, encoding='utf-8') as file:
        for student_and_class in csv.reader(file, delimiter=";"):
            ...
            people, created = await Peoples.objects.aget_or_create(nickname=student, name=student, password="0000",
                                                                   school='МАОУ "Лицей №6"')
            await Students.objects.aget_or_create(nickname=people, clazz=clazz, grouping=group)
