...
function getLessonContainer(id, clazz, subject, classroom, homework) {
    ...
    if (clazz) {
        ...
        setSwal(lessonContainer, subject, `</br><b>Класс: </b> ${clazz}</br>
...
function createSchedule(schedule) {
    for (let i = 0; i < 6; i++) {
        ...
        for (let j = 0; j < 8; j++) {
            ...
            day.append(getLessonContainer(str(i, j), clazz, subject, lesson[2], homework))
            ...
            if (button) {
                button.onclick = function (event) {
                    ...
                    Swal.mixin({
                        ...
                        html: ` <br>
                                <textarea class="homeworkInputArea" id="homeworkInputArea">${homework}</textarea>`,
                        confirmButtonText: '<i>Сохранить</i>',
                        showLoaderOnConfirm: true,
                        preConfirm: () => {
                            return xmlHttpRequest(clazz, document.getElementById("homeworkInputArea").value, i, j).then(function (text) {
                                getSchedule()
                                Swal.fire({
                                    title: text,
                                    icon: 'success',
